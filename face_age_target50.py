...
# ### 넘파이로 세이브하기
# np.save('./_save/_npy/proj_faceage_bat50_x_train.npy', arr=x_train)
# np.save('./_save/_npy/proj_faceage_bat50_x_test.npy', arr=x_test)
# np.save('./_save/_npy/proj_faceage_bat50_y_train.npy', arr=y_train)
# np.save('./_save/_npy/proj_faceage_bat50_y_test.npy', arr=y_test)
# np.save('./_save/_npy/proj_faceage_bat50_x_pred.npy', arr=x_pred)
# np.save('./_save/_npy/proj_faceage_bat50_y_pred.npy', arr=y_pred)


# ============================================================================================================

### 데이터 로드하기
x_train = np.load('./_save/_npy/proj_faceage_bat50_x_train.npy')
x_test = np.load('./_save/_npy/proj_faceage_bat50_x_test.npy')
y_train = np.load('./_save/_npy/proj_faceage_bat50_y_train.npy')
y_test = np.load('./_save/_npy/proj_faceage_bat50_y_test.npy')
x_pred = np.load('./_save/_npy/proj_faceage_bat50_x_pred.npy')
y_pred = np.load('./_save/_npy/proj_faceage_bat50_y_pred.npy')
...
'''
    x_train.shape: (50880, 50, 50, 3)
    x_test.shape: (220, 50, 50, 3)
    y_train.shape: (50880, 11)
    y_test.shape: (220, 11)
    x_pred.shape: (11, 50, 50, 3)
    y_pred.shape: (11, 11)
'''
...
x_pred = x_pred.reshape(x_pred.shape[0], 50*50*3)
...
cp = ModelCheckpoint(monitor='val_loss', mode='auto', save_best_only=True,
                     filepath='./_save/ModelCheckPoint/face_age_MCP3_batch50_4.hdf5')
...
model.save('./_save/ModelCheckPoint/face_age_model_save_batch50_4.h5')
...
*augment 50000
'./_save/ModelCheckPoint/face_age_model_save_aug5_10.h5'
걸린시간 : 486.0002839565277
acc : 0.7687644958496094
val_acc : 0.7146226167678833
loss : 0.12272727489471436
val_loss : 0.9266372919082642
[9, 3, 6, 3, 10, 5, 5, 7, 1, 9, 3] 4개


*Image batch 50*50
'./_save/ModelCheckPoint/face_age_model_save_batch50_1.h5'
걸린시간 : 585.5190761089325
acc : 0.6392130255699158
val_acc : 0.5483490824699402
loss : 0.10000000149011612
val_loss : 1.3705002069473267
[5, 4, 8, 8, 2, 6, 10, 10, 1, 1, 10] 1개

'./_save/ModelCheckPoint/face_age_model_save_batch50_2.h5'
걸린시간 : 444.9297561645508
acc : 0.5013240575790405
val_acc : 0.4716981053352356
loss : 0.08636363595724106
val_loss : 1.5368735790252686
[1, 1, 7, 4, 4, 7, 4, 6, 9, 10, 7] 2개

'./_save/ModelCheckPoint/face_age_model_save_batch50_3.h5'
걸린시간 : 390.66185688972473
acc : 0.502648115158081
val_acc : 0.4500786066055298
loss : 0.09090909361839294
val_loss : 1.589906930923462
[2, 9, 2, 0, 2, 2, 1, 3, 3, 3, 3] 1개
'''
